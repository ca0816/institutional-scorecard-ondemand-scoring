...
class AuditorFeatures:
    ...
    def prepare_dummy_records(company_ids: list, min_yr, max_yr):
        ...
        result_df['company_master_id'] = pd.to_numeric(result_df['company_master_id'], errors='coerce')
    ...
    def build_agg_features(self, data: pd.DataFrame, feature_prefix, feature_col, agg_type='sum'):
        ...
        result = data.assign(
            **{
                f'{feature_prefix}_1yrs': data.groupby('company_master_id', group_keys=False)[feature_col].apply(
                    lambda x: x.rolling(window=1).agg(agg_func)
                ),
                f'{feature_prefix}_2yrs': data.groupby('company_master_id', group_keys=False)[feature_col].apply(
                    lambda x: x.rolling(window=2).agg(agg_func)
                ),
                f'{feature_prefix}_3yrs': data.groupby('company_master_id', group_keys=False)[feature_col].apply(
                    lambda x: x.rolling(window=3).agg(agg_func)
                ),
                f'{feature_prefix}_4yrs': data.groupby('company_master_id', group_keys=False)[feature_col].apply(
    ...
    def get_auditor_features(self):
        ...
        form_aoc4_dtls = form_aoc4_dtls[
            (form_aoc4_dtls['financial_year'].isin(self.valid_years)) &
            (form_aoc4_dtls['valid_data'] == 1)
            ]
        form_aoc4_dtls = form_aoc4_dtls.drop(columns=['valid_data'])
        ...
        form_aoc4_dtls = form_aoc4_dtls[form_aoc4_dtls['row'] == 1].drop(columns=['inserted_at', 'row'])
        ...
        aoc4_auditor_dtls = aoc4_auditor_dtls[
            (aoc4_auditor_dtls['financial_year'].isin(self.valid_years))
        ].drop_duplicates()
        ...
        aoc4xbrl_auditor_dtls = aoc4xbrl_auditor_dtls[
            aoc4xbrl_auditor_dtls['financial_year'].isin(self.valid_years)
        ].drop_duplicates()
        ...
        auditor_resignation = auditor_resignation.drop(columns=['aud_resign_month', 'aud_resign_year'])
        auditor_resignation = auditor_resignation[
            auditor_resignation['financial_year'].isin(self.valid_years)
        ]
        ...
        auditor_dtls['adv_qualified'] = pd.to_numeric(auditor_dtls['not_qualified'])
        ...
        caro_standalone = aoc4_caro_dtls[
            ...
            (aoc4_caro_dtls['financial_year'].isin(self.valid_years))
            ]
